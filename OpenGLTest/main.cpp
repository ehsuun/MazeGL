...
glm::vec3 lastCameraPos = glm::vec3(0.0f, 0.0f, -5.1f);
...
int main()
{
	...
	*/
	...
	glGenVertexArrays(1, &VAtest);
	glGenBuffers(1, &VBtest);

	// 1. Bind Vertex Array Object
	glBindVertexArray(VAtest);
	// 2. Copy our vertices array in a vertex buffer for OpenGL to use
	glBindBuffer(GL_ARRAY_BUFFER, VBtest);
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat)*30, &testVerts[0], GL_STATIC_DRAW);
	// 3. Copy our index array in a element buffer for OpenGL to use
	//glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	//glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, s.Textures[0]);
		glUniform1i(glGetUniformLocation(shaders[0].Program, "ourTexture"), 0);

		//Sending Matrix Uniforms to shader
		GLint modelLoc = glGetUniformLocation(shaders[0].Program, "model");
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

		GLint viewLoc = glGetUniformLocation(shaders[0].Program, "view");
		glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));

		GLint projectionLoc = glGetUniformLocation(shaders[0].Program, "projection");
		glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, glm::value_ptr(projection));


		glBindVertexArray(VAtest);
		glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
		glDrawArrays(GL_TRIANGLES, 0, 30 / 5);
		//glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		glBindVertexArray(0);
